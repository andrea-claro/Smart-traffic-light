...
public class AgentManager : Agent
{
    ...
    [SerializeField] private float semaphoreTimer = 20f;
    [SerializeField] [Range(0.1f, 5f)] private float timerForDecision = 0.5f;
    ...
    public override void OnActionReceived(float[] vectorAction)
    {
        switch (vectorAction[0])
        {
            ...
            case 0:
                StartCoroutine(SemaphoreYellowToRed(_semaphores[SouthSemaphore]));
                break;

            //Accende semaforo a SUD e spegne semaforo a NORD
            case 1:
                StartCoroutine(SemaphoreYellowToRed(_semaphores[NorthSemaphore]));
                break;
        }
    }
    
    private IEnumerator SemaphoreYellowToRed(SemaphoreController semaphore)
    {
        if (!_semaphores[SouthSemaphore].IsGreen && !_semaphores[NorthSemaphore].IsGreen)
        {
            if (semaphore.gameObject.tag.Equals("south"))
            {
        ...
        else
        {
            ...
            {
                semaphore.IsYellow = true;
                
                if (semaphore.gameObject.tag.Equals("south"))
                {
                    _uiController.southSemaphoreValue.color = Color.yellow;

                    yield return new WaitForSeconds(timerForDecision/2);
            
                    semaphore.IsYellow = false;
                ...
                {
                    _uiController.northSemaphoreValue.color = Color.yellow;
